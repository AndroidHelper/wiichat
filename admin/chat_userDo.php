...
<?php
	...
	if(empty($_POST["id_list"])){
		alertInfo2('请选择删除项!','chat_userList.php?'.$url,0);
	}
	$id_list=$_POST["id_list"];
		foreach($id_list as $val){
			...
			if($row)
			{
				...
				{
					alertInfo2('用户和房主存在关联，请先删除相关联的房主信息。','chat_userList.php?'.$url,0);
				}else{
					$sql3="delete from wiichat_user where user_id=".$val."";
					if(!mysql_query($sql3)){
						alertInfo2('删除失败!','chat_userList.php?'.$url,0);
					}
				}
			}else{
				alertInfo2('删除失败，原因该用户不存在。','chat_userList.php?'.$url,0);
			}
		}
		alertInfo2('删除成功!','chat_userList.php?'.$url,0);
	}
	if($action=='del')
	{
		$id2=$_GET['id2'];
		if(!empty($id2))
		{
			...
			if($row)
			{
				...
				if(mysql_num_rows($result2)>0)
				{
					alertInfo2('该用户和房主存在关联，请先删除相关联的房主信息。','chat_userList.php?'.$url,0);
				}else{
	...
	if($i==0)
	{
		alertInfo2('当前没有需要保存的内容','chat_userList.php?'.$url,0);
	}
	$i=checkData2($i,"11",0);
	for($x=1;$x<=$i;$x++){
		$tempId=$_POST["id".$x];
		...
		if(!mysql_query($sqlStrt)){
			alertInfo2('保存失败! 原因：SQL更新失败','chat_userList.php?'.$url,0);
		}
	}
	alertInfo2('保存成功!','chat_userList.php?'.$url,0);
}
?>

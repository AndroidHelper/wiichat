...
<?php
	...
	$sqlStrt="select * from wiichat_user where user_account like '%".$ac."%' and user_mobile like '%".$m."%' and user_email like '%".$mail."%' order by user_id desc";
	$sqlStr="select * from wiichat_user where user_account like '%".$ac."%' and user_mobile like '%".$m."%' and user_email like '%".$mail."%' order by user_id desc limit $startRow,$pagesize";			
?>
	<div class="bgintor">
		<div class="tit1">
			<ul>
				<li><a href="chat_user.php" target="mainFrame" >用户搜索</a> </li>
				<li class="l1"><a href="chat_userByRights.php" target="mainFrame">按权限查看</a> </li>
			</ul>		
		</div>
		<div class="listintor">
			<div class="header1"><img src="images/square.gif" width="6" height="6" alt="" />
				<span>位置：聊天室管理 －&gt; <strong><?php echo $sortName;?> 聊天室用户搜索</strong></span></div>
			<form  name="listForm" method="post" action="#" id="listForm">
				<div class="header2"><span><?php echo $sortName;?> 聊天室用户搜索</span></div>
				<div class="header3">
					<input type="checkbox" id="handler" name="handler" onClick="checkAll(this.form)"> <strong>全选</strong><a href="javascript:document.listForm.action='chat_userDo.php?action=rmd&<?php echo $url?>&page=<?php echo $page?>';document.listForm.submit();"><img src="images/act_save.gif" width="16" height="16" alt="保存" /> <strong>保存</strong> </a> <a href="javascript:if(confirm('您确定要删除吗？')){document.listForm.action='chat_userDo.php?action=delAll&<?php echo $url?>';document.listForm.submit();}"><img src="images/act_del.gif" width="14" height="14" alt="删除" /> <strong>删除</strong> </a>
				</div>
				<div class="content">
					<table width="100%">
						<tr class="t1">
						...
						<?php
						  ...
						  <tr>
							...
							<td><a href="javascript:if(confirm('您确定要删除吗？')){location.href='chat_userDo.php?action=del&<?php echo $url?>&id2=<?php echo $row["user_id"]?>'}"><img src="images/dot_del.gif" width="9" height="9" alt="删除" /></a></td>
						  </tr>
						  <?php
								 }
						  ?>

	...
	function doSend(url){
			...
			http_request.send("t="+x+"&dabasename="+escape(databaseName)+"&dataaddress="+databaseIP+"&datauser="+escape(databaseUser)+"&datapwd="+escape(databasePWD));
		}
		//处理返回的信息
			...
			if (http_request.readyState==4)
			{
				if (http_request.status==200){	
					...
					if (txt=="S"){
						...
						if (x>=t)
							document.getElementById("success").innerHTML="<p><input class='input1' type='image' src='images/next.gif' onclick='javascript:location.href=\"install_admin.php?dataaddress="+databaseIP+"&datauser="+escape(databaseUser)+"&dabasename="+escape(databaseName)+"&datapwd="+escape(databasePWD)+"\"' value='下一步'/></p>";
						if (x<t)
						{
							x++;
							setTimeout("act()",1000);
